        ...
        return expression switch
        {
            SqlBinaryExpression be => be.Operation switch
            {
                "%" => new SqlFunction(
                    be.SystemType,
                    "Mod",
                    !be.Expr1.SystemType!.IsIntegerType()
                        ? new SqlExpression(typeof(long), "{0}::BIGINT", Precedence.Primary, be.Expr1)
                        : be.Expr1,
                    be.Expr2
                ),
            ...
            {
                "Convert" when func.SystemType.ToUnderlying() == typeof(bool) => AlternativeConvertToBoolean(func, 1) ??
                                                                                 new SqlExpression(func.SystemType, "Cast({0} as {1})",
                                                                                     Precedence.Primary, FloorBeforeConvert(func),
                                                                                     func.Parameters[0]),
                    ...
                    : Add<int>(
                        new SqlExpression(func.SystemType, "Position({0} in {1})", Precedence.Primary, func.Parameters[0],
                            ConvertExpressionImpl(
                                new SqlFunction(typeof(string), "Substring", func.Parameters[1], func.Parameters[2],
                                    Sub<int>(ConvertExpressionImpl(new SqlFunction(typeof(int), "Length", func.Parameters[1]), visitor),
                                        func.Parameters[2])), visitor)), Sub(func.Parameters[2], 1)),
                _ => expression,
            },
            _ => expression
        };
